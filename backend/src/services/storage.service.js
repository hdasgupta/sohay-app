...
export const isR2Configured = () => Boolean(env.r2.accountId && env.r2.accessKeyId && env.r2.secretAccessKey && env.r2.bucket);
...
export async function putPrescription(key, buffer) {
  ...
  await getPrescriptionClient().send(new PutObjectCommand({
    Bucket: env.storage.bucket, Key: key, Body: buffer, ContentType: 'application/pdf',
  }));
...
export async function getPrescription(key) {
  ...
  try {
    const res = await getPrescriptionClient().send(new GetObjectCommand({ Bucket: env.storage.bucket, Key: key }));
...
export async function deletePrescription(key) {
  if (!key) return;
  if (env.storage.provider === 'memory') { memoryStore.delete(key); return; }
  await getPrescriptionClient().send(new DeleteObjectCommand({ Bucket: env.storage.bucket, Key: key }));
  logger.info(`Prescription deleted from storage: ${key}`);
}

/** Stream a recording (readable stream) to Cloudflare R2 */
export async function putRecordingStream(key, stream, contentType = 'video/mp4') {
  if (!isR2Configured()) throw new Error('Cloudflare R2 is not configured');
  const upload = new Upload({
    client: getR2Client(),
    params: { Bucket: env.r2.bucket, Key: key, Body: stream, ContentType: contentType },
    queueSize: 4,
    partSize: 8 * 1024 * 1024,
  });
  await upload.done();
  const url = env.r2.publicBaseUrl ? `${env.r2.publicBaseUrl}/${key}` : `r2://${env.r2.bucket}/${key}`;
