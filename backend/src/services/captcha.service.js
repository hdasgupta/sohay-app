...
function randomText() {
  ...
  for (let i = 0; i < LENGTH; i += 1)
    s += CAPTCHA_CHARSET[crypto.randomInt(CAPTCHA_CHARSET.length)];
  return s;
}
...
export function renderCaptcha(text) {
  ...
  for (let y = 0; y < HEIGHT; y += 1) {
    for (let x = 0; x < WIDTH; x += 1) {
      ...
      const i = (y * WIDTH + x) * 3;
      for (let c = 0; c < 3; c += 1)
        buf[i + c] = c1[c] * (1 - t) + c2[c] * t + rand(-18, 18);
    }
  }
  const plot = (x, y, color, alpha = 1) => {
    ...
    for (let c = 0; c < 3; c += 1)
      buf[i + c] = buf[i + c] * (1 - alpha) + color[c] * alpha;
  };
  ...
  [...text].forEach((ch, idx) => {
    ...
    const segment = (a, b) => {
      const [x1, y1] = toCanvas(a.x, a.y);
      const [x2, y2] = toCanvas(b.x, b.y);
      const steps = Math.max(1, Math.ceil(Math.hypot(x2 - x1, y2 - y1) * 1.5));
      for (let k = 0; k <= steps; k += 1) {
        const t = k / steps;
        disc(
          x1 + (x2 - x1) * t + rand(-0.3, 0.3),
          y1 + (y2 - y1) * t + rand(-0.3, 0.3),
          radius,
          color,
        );
      }
    ...
    for (const cell of cells) {
      ...
      const right = lit.has(`${gx + 1},${gy}`);
      const down = lit.has(`${gx},${gy + 1}`);
      if (right) segment(cell, { x: cell.x + 1, y: cell.y });
      if (down) segment(cell, { x: cell.x, y: cell.y + 1 });
      if (lit.has(`${gx + 1},${gy + 1}`) && !right && !down)
        segment(cell, { x: cell.x + 1, y: cell.y + 1 });
      if (
        lit.has(`${gx - 1},${gy + 1}`) &&
        !down &&
        !lit.has(`${gx - 1},${gy}`)
  ...
  for (let n = 0; n < 3; n += 1) {
    ...
    const x0 = rand(0, WIDTH * 0.2);
    const x3 = rand(WIDTH * 0.8, WIDTH);
    const pts = [
      [x0, rand(0, HEIGHT)],
      [rand(0, WIDTH), rand(-20, HEIGHT + 20)],
      [rand(0, WIDTH), rand(-20, HEIGHT + 20)],
      [x3, rand(0, HEIGHT)],
    ];
    const thick = 1;
  ...
  for (let n = 0; n < 600; n += 1)
    plot(
      rand(0, WIDTH),
      rand(0, HEIGHT),
      hsl(rand(0, 360), 0.6, rand(0.2, 0.7)),
      rand(0.4, 1),
    );

  // global sine warp
  const out = new PNG({ width: WIDTH, height: HEIGHT });
  const ax = rand(2.5, 4.5);
  const ay = rand(2, 4);
  const px = rand(0, Math.PI * 2);
  const py = rand(0, Math.PI * 2);
  const lx = rand(18, 30);
  const ly = rand(26, 40);
  for (let y = 0; y < HEIGHT; y += 1) {
    for (let x = 0; x < WIDTH; x += 1) {
      const sx = Math.min(
        WIDTH - 1,
        Math.max(0, Math.round(x + ax * Math.sin(y / lx + px))),
      );
      const sy = Math.min(
        HEIGHT - 1,
        Math.max(0, Math.round(y + ay * Math.sin(x / ly + py))),
      );
...
export async function createCaptcha() {
  ...
  const { rows } = await query(COMMON_SQL.CAPTCHA_INSERT, [
    hashAnswer(text),
    env.captchaValidityMinutes,
  ]);
...
export async function verifyCaptcha(captchaId, captchaText) {
  if (!captchaId || !captchaText)
    throw AppError.badRequest("Please enter the captcha");
  if (!/^[0-9a-f-]{36}$/i.test(String(captchaId)))
    throw AppError.badRequest("Invalid captcha, please refresh it");
  const { rows } = await query(COMMON_SQL.CAPTCHA_CONSUME, [captchaId, false]);
  if (!rows.length)
    throw AppError.badRequest("Captcha expired, please refresh and try again");
...
export function peekCaptchaAnswer(id) {
  if (!env.isTest)
    throw new Error("peekCaptchaAnswer is only available in tests");
