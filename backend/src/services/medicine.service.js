...
export function ensureMedicines() {
  ...
  running = (async () => {
    try {
      ...
      if (count > 0) {
        logger.info(
          `Medicine table already has ${count} rows - skipping import`,
        );
        return count;
      }
      logger.info(`Medicine table empty - downloading ${env.medicineCsvUrl}`);
      const res = await axios.get(env.medicineCsvUrl, {
        responseType: "stream",
        timeout: 120000,
      });
      ...
      await query(SYSTEM_SQL.META_UPSERT, [
        META_KEYS.MEDICINES_IMPORTED,
        String(total),
      ]);
      logger.info(
        `Medicine import finished: read ${read} rows, ${total} unique medicines stored`,
      );
      return total;
    } catch (err) {
