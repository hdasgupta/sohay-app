...
export function verifyJaasSignature(
  rawBody,
  header,
  secret = env.jaas.webhookSecret,
  nowSec = Math.floor(Date.now() / 1000),
) {
  if (!secret) {
    if (env.isProduction)
      logger.warn(
        "JAAS_WEBHOOK_SECRET not set - webhook signature NOT verified",
      );
    return true;
  }
  ...
  if (!ts || !v1.length)
    throw AppError.unauthorized("Malformed webhook signature");
  if (Math.abs(nowSec - ts) > TOLERANCE_SECONDS)
    throw AppError.unauthorized("Webhook timestamp outside tolerance");
...
async function transfer(recordingId, appointmentId, sourceUrl, sessionId) {
  try {
    if (!isR2Configured())
      throw new Error("Cloudflare R2 not configured (set R2_* variables)");
    const res = await axios.get(sourceUrl, {
      responseType: "stream",
      timeout: 10 * 60 * 1000,
    });
    const key = `recordings/${todayInKolkata()}/appointment-${appointmentId || "unknown"}-${sessionId || recordingId}.mp4`;
    const stored = await putRecordingStream(
      key,
      res.data,
      res.headers["content-type"] || "video/mp4",
    );
    await query(SYSTEM_SQL.RECORDING_UPDATE, [
      recordingId,
      stored.key,
      stored.url,
      "uploaded",
...
export async function handleJaasEvent(event) {
  ...
  const { rows } = await query(SYSTEM_SQL.RECORDING_INSERT, [
    appointmentId,
    fqn,
    String(event.idempotencyKey || crypto.randomUUID()),
    data.recordingSessionId || null,
    Number.isFinite(data.durationSec) ? Math.round(data.durationSec) : null,
    data.preAuthenticatedLink || null,
    "pending",
  ]);
