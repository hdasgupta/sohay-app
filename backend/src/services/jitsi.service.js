...
export function createMeetingToken(user, roomId, isModerator) {
  if (!env.jaas.privateKey || !env.jaas.appId || !env.jaas.apiKeyId) throw new AppError('Video service is not configured', 500);
  ...
  const payload = {
    ...
    exp: now + 3 * 60 * 60,
    context: {
      user: { id: String(user.id), name: user.name, email: user.email, avatar: '', moderator: isModerator ? 'true' : 'false' },
      features: {
        livestreaming: 'false',
  ...
  return jwt.sign(payload, env.jaas.privateKey, { algorithm: 'RS256', header: { kid: env.jaas.apiKeyId, typ: 'JWT', alg: 'RS256' } });
}
