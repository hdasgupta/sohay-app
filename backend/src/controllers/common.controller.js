...
export async function login(req, res) {
  ...
  if (
    !user ||
    !(await verifyPassword(String(req.body.password), user.password_hash))
  ) {
...
export async function sendOtp(req, res) {
  ...
  if (purpose === OTP_PURPOSE.REGISTER && existing)
    throw AppError.conflict("This email address is already registered");
  if (purpose === OTP_PURPOSE.RESET && !existing)
    throw AppError.notFound("No account exists with this email address");
  const out = await issueOtp(email, purpose);
  ok(
    res,
    out,
    `OTP sent to ${email}. It is valid for ${env.otpValidityMinutes} minutes`,
  );
}
...
export async function checkOtp(req, res) {
  ...
  const email = cleanEmail(req.body.email);
  if (!Object.values(OTP_PURPOSE).includes(req.body.purpose))
    throw AppError.badRequest("Invalid OTP purpose");
...
export async function registerPatient(req, res) {
  ...
  if (await common.findUserByEmail(email))
    throw AppError.conflict("This email address is already registered");
  ...
  const user = await withTransaction(async (client) => {
    ...
    await patientModel.insertPatient(client, {
      ...
      contactNumber: String(b.contactNumber).replace(/[\s-]/g, ""),
    });
    return u;
  });
...
export async function resetPassword(req, res) {
  ...
  const updated = await common.updatePasswordByEmail(
    email,
...
function assertAppointmentAccess(user, appt) {
  ...
  const allowed =
    ...
    (user.role === ROLES.PATIENT &&
      (Number(appt.patient_id) === uid || Number(appt.booked_by) === uid));
  if (!allowed)
...
export async function meetingToken(req, res) {
  ...
  if (req.user.role === ROLES.ADMIN)
    throw AppError.forbidden(
      "Only the doctor and the patient can join the consultation",
    );
  if (appt.status === APPOINTMENT_STATUS.CANCELLED)
    throw AppError.badRequest("This appointment has been cancelled");
  const now = nowInKolkata();
  const dto = appointmentDto(appt);
  if (appt.appointment_date !== now.date) {
    throw AppError.badRequest(
      `The consultation room opens on ${appt.appointment_date} at ${dto.startTime} (IST)`,
      { appointment: dto },
    );
  }
  if (
    timeToMinutes(now.time) <
    timeToMinutes(dto.startTime) - JOIN_EARLY_MINUTES
  ) {
    throw AppError.badRequest(
      `The consultation room opens ${JOIN_EARLY_MINUTES} minutes before ${dto.startTime} (IST)`,
      { appointment: dto },
    );
  }
  ...
  logger.info(
    `Meeting token issued: appointment ${id} user ${req.user.id} (${req.user.role})`,
  );
  ok(res, {
