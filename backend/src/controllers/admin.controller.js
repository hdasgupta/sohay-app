...
import {
  requireFields, cleanEmail, cleanString, assertEmail, assertPassword, assertSex, toId,
} from '../utils/validators.js';
import { doctorDto, patientDto, appointmentDto } from '../utils/serializers.js';
import { ROLES, APPOINTMENT_STATUS, ACTIVE_APPOINTMENT_STATUSES } from '../config/constants.js';
import { nowInKolkata, isValidIsoDate, formatDisplayDate, formatDisplayTime, normaliseTime, isFutureSlot } from '../utils/date.js';
...
function readSignature(signature) {
  ...
  if (typeof signature !== 'string' || signature.length > MAX_SIGNATURE_LENGTH || !dataUrlToBuffer(signature)) {
    throw AppError.badRequest('Signature must be a PNG or JPEG image smaller than 500 KB');
  }
  return signature;
}
...
function readDoctorBody(body, { requirePassword }) {
  ...
  if (name.length < 2) throw AppError.badRequest('Doctor name must be at least 2 characters');
  if (speciality.length < 2) throw AppError.badRequest('Speciality is required');
  assertEmail(email);
  assertSex(body.sex);
  const hasPassword = Boolean(body.password || body.confirmPassword);
  if (requirePassword || hasPassword) assertPassword(body.password || '', body.confirmPassword || '');
  const availability = validateAvailability(body.availability);
  return { name, email, speciality, sex: body.sex, password: hasPassword ? body.password : null, availability, signature: readSignature(body.signature) };
}
...
export async function createDoctor(req, res) {
  ...
  if (await common.findUserByEmail(data.email)) throw AppError.conflict('This email address is already registered');
  const passwordHash = await hashPassword(data.password);
  const id = await withTransaction(async (client) => {
    const u = await common.insertUser({ name: data.name, email: data.email, passwordHash, role: ROLES.DOCTOR }, client);
    await adminModel.insertDoctor(client, { userId: u.id, sex: data.sex, speciality: data.speciality, signature: data.signature });
...
export async function updateDoctor(req, res) {
  ...
  if (clash && Number(clash.id) !== id) throw AppError.conflict('This email address is already registered');
  const passwordHash = data.password ? await hashPassword(data.password) : null;
  await withTransaction(async (client) => {
    await adminModel.updateUserNameEmail(client, { userId: id, name: data.name, email: data.email });
    await adminModel.updateDoctor(client, { userId: id, sex: data.sex, speciality: data.speciality, signature: data.signature });
    if (passwordHash) await adminModel.updateUserPassword(client, id, passwordHash);
...
export async function setDoctorStatus(req, res) {
  const id = toId(req.params.id, 'doctor id');
  if (typeof req.body.disabled !== 'boolean') throw AppError.badRequest('disabled must be true or false');
  const row = await adminModel.setUserDisabled(id, req.body.disabled, ROLES.DOCTOR);
  if (!row) throw AppError.notFound('Doctor not found');
  logger.info(`Doctor ${id} ${row.is_disabled ? 'disabled' : 'enabled'} by admin ${req.user.id}`);
  ok(res, { id: Number(row.id), isDisabled: row.is_disabled }, `Doctor ${row.name} ${row.is_disabled ? 'disabled' : 'enabled'}`);
}
...
export async function upcomingAppointments(req, res) {
  ...
  const rows = await adminModel.upcomingForPatientDoctor(patientId, doctorId, ACTIVE_APPOINTMENT_STATUSES, now.date, now.time);
...
export async function slotsForReschedule(req, res) {
  ...
  const slots = await availableSlots({ doctorId: appt.doctor_id, date, patientId: appt.patient_id, excludeAppointmentId: appointmentId });
  ok(res, slots);
}
...
export async function reschedule(req, res) {
  ...
  if (!appt) throw AppError.notFound('Appointment not found');
  if (!ACTIVE_APPOINTMENT_STATUSES.includes(appt.status)) throw AppError.badRequest(`A ${appt.status} appointment cannot be rescheduled`);
  if (!isFutureSlot(appt.appointment_date, normaliseTime(appt.start_time))) throw AppError.badRequest('Only upcoming appointments can be rescheduled');
  if (appt.appointment_date === date && normaliseTime(appt.start_time) === startTime) throw AppError.badRequest('Please choose a different date or time slot');
  await getDoctorForBooking(appt.doctor_id);
  await assertSlotBookable({ doctorId: appt.doctor_id, patientId: appt.patient_id, date, startTime, excludeAppointmentId: id });
  const done = await withTransaction((client) => adminModel.rescheduleAppointment(client, {
    id, date, startTime, endTime: slotEnd(startTime), status: APPOINTMENT_STATUS.RESCHEDULED, allowedStatuses: ACTIVE_APPOINTMENT_STATUSES,
  }));
  if (!done) throw AppError.conflict('Appointment could not be rescheduled (status changed)');
  ...
  logger.info(`Appointment ${id} rescheduled to ${date} ${startTime} by admin ${req.user.id}`);
