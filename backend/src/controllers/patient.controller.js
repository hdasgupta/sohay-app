...
async function bookableMembers(userId) {
  ...
  const members = (await patientModel.familyMembers(me.family_id)).map(
    patientDto,
  );
...
async function assertCanBookFor(userId, patientId) {
  ...
  if (!members.some((m) => m.id === Number(patientId)))
...
export async function members(req, res) {
  ...
  ok(res, {
    family: family ? { id: Number(family.id), name: family.name } : null,
    members: list,
  });
}
...
export async function slots(req, res) {
  ...
  const patientId = req.query.patientId
    ? toId(req.query.patientId, "patient id")
    : Number(req.user.id);
...
export async function book(req, res) {
  ...
  if (Number(req.user.id) !== patientId)
    sendEmail({ to: req.user.email, ...mail });
  logger.info(
    `Appointment ${row.id} booked: patient ${patientId} doctor ${doctorId} ${date} ${startTime} by ${req.user.id}`,
  );
  created(
    res,
    { ...appointmentDto(appt), meetingUrl },
...
export async function listAppointments(req, res) {
  ok(
    res,
    (await patientModel.appointmentsForPatient(req.user.id)).map(
      appointmentDto,
    ),
  );
}
...
export async function cancel(req, res) {
  ...
  if (Number(appt.patient_id) !== uid && Number(appt.booked_by) !== uid)
    throw AppError.forbidden("You cannot cancel this appointment");
  ...
  if (!isFutureSlot(appt.appointment_date, normaliseTime(appt.start_time)))
    throw AppError.badRequest("Only upcoming appointments can be cancelled");
  const done = await patientModel.cancelAppointment({
    id,
  ...
  sendEmail({
    ...
    ...appointmentCancelledEmail({
      ...
      dateLabel: formatDisplayDate(appt.appointment_date),
      timeLabel: formatDisplayTime(appt.start_time),
    }),
  });
  logger.info(`Appointment ${id} cancelled by ${uid}`);
  ok(
    res,
    appointmentDto(await common.getAppointmentById(id)),
...
export async function getFamily(req, res) {
  ...
  if (me.family_id) {
    ...
    family = {
      id: Number(f.id),
      name: f.name,
      createdBy: Number(f.created_by),
      createdByName: f.created_by_name,
      createdAt: f.created_at,
    };
    memberList = (await patientModel.familyMembers(me.family_id)).map(
      patientDto,
    );
...
export async function createFamily(req, res) {
  ...
  if (name.length < 2)
    throw AppError.badRequest("Family name must be at least 2 characters");
  const me = await common.getPatientProfile(req.user.id);
  ...
  created(
    res,
    { id: Number(fam.id), name: fam.name },
    `Family "${name}" created`,
  );
}
...
export async function invite(req, res) {
  ...
  const me = await common.getPatientProfile(req.user.id);
  if (!me.family_id)
    throw AppError.badRequest("Create a family before inviting members");
  const invitee = await patientModel.findPatientByEmail(email);
  if (!invitee)
    throw AppError.notFound("No registered patient exists with this email");
  if (Number(invitee.id) === Number(req.user.id))
    throw AppError.badRequest("You cannot invite yourself");
  if (invitee.is_disabled)
    throw AppError.badRequest("This patient account is disabled");
  if (invitee.family_id && Number(invitee.family_id) === Number(me.family_id))
    throw AppError.conflict("This patient is already a member of your family");
  if (invitee.family_id)
    throw AppError.conflict("This patient is already part of another family");
  const inv = await patientModel.insertInvitation({
  ...
  sendEmail({
    ...
    ...familyInvitationEmail({
      ...
      appUrl: `${env.frontendUrl}/patient/family`,
    }),
  });
...
export async function respond(req, res) {
  ...
  if (!inv || Number(inv.invitee_id) !== Number(req.user.id))
    throw AppError.notFound("Invitation not found");
  if (!inv.is_open)
    throw AppError.badRequest("This invitation is no longer open");
  ...
  await withTransaction(async (client) => {
    if (accepted) {
      ...
      const closed = await patientModel.closeInvitation(
        client,
        id,
      ...
      await patientModel.closeOtherInvitations(
        client,
        req.user.id,
  ...
  logger.info(
    `Invitation ${id} ${accepted ? "accepted" : "rejected"} by ${req.user.id}`,
  );
  ok(
    res,
    null,
    accepted
      ? `You joined the family "${inv.family_name}"`
      : "Invitation rejected",
  );
}
...
export async function cancelInvitation(req, res) {
  ...
  const inv = await patientModel.getInvitation(id);
  const me = await common.getPatientProfile(req.user.id);
  if (!inv || !me.family_id || Number(inv.family_id) !== Number(me.family_id))
    throw AppError.notFound("Invitation not found");
  const closed = await patientModel.closeInvitation(
    null,
    id,
...
export async function leaveFamily(req, res) {
  const me = await common.getPatientProfile(req.user.id);
  if (!me.family_id)
    throw AppError.badRequest("You are not part of any family");
  await withTransaction(async (client) => {
    await patientModel.clearPatientFamily(client, req.user.id);
    if ((await patientModel.familyMemberCount(client, me.family_id)) === 0)
      await patientModel.deleteFamily(client, me.family_id);
  });
