...
export async function cancelPastAppointments(today = todayInKolkata()) {
  const { rows } = await query(SYSTEM_SQL.CANCEL_PAST_APPOINTMENTS, [
  ...
  logger.info(
    `Midnight job: ${rows.length} past appointment(s) marked cancelled`,
  );
  return rows.length;
}
...
export function startAppointmentStatusJob() {
  ...
  task = cron.schedule(
    "0 0 * * *",
    () => {
      cancelPastAppointments().catch((err) =>
        logger.error("Midnight job failed", err.message),
      );
    },
  ...
  cancelPastAppointments().catch((err) =>
    logger.error("Startup catch-up job failed", err.message),
  );
  return task;
}
export function stopAppointmentStatusJob() {
  if (task) {
    task.stop();
    task = null;
  }
