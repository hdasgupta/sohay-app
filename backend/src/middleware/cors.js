...
const allowed = new Set([env.frontendUrl, ...env.corsOrigins].map((o) => o.replace(/\/+$/, '')));
const allowAll = env.corsOrigins.includes('*');

export const corsMiddleware = cors({
  origin(origin, cb) {
    if (!origin || allowAll || allowed.has(origin)) return cb(null, true);
    if (env.corsAllowVercelPreviews && /^https:\/\/[a-z0-9-]+\.vercel\.app$/i.test(origin)) return cb(null, true);
    if (!env.isProduction && /^http:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/.test(origin)) return cb(null, true);
