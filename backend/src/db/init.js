...
import {
  ROLES, APPOINTMENT_STATUS, INVITATION_STATUS, FOOD_TIMING, FOOD_TIMING_LABEL, META_KEYS, SCHEMA_VERSION,
...
export async function isInitialised() {
  const { rows } = await query(SYSTEM_SQL.TABLE_EXISTS, [`${PUBLIC_SCHEMA}.app_meta`]);
...
async function dropObjectsIndividually(client) {
  const { rows: tables } = await client.query(SYSTEM_SQL.LIST_SCHEMA_TABLES, [PUBLIC_SCHEMA]);
  for (const t of tables) await client.query(SYSTEM_SQL.DROP_TABLE_CASCADE(pg.escapeIdentifier(t.tablename)));
  const { rows: types } = await client.query(SYSTEM_SQL.LIST_SCHEMA_TYPES, [PUBLIC_SCHEMA, ['e', 'c', 'd']]);
  for (const t of types) await client.query(SYSTEM_SQL.DROP_TYPE_CASCADE(pg.escapeIdentifier(t.typname)));
}
...
export async function resetSchema() {
  ...
  try {
    logger.warn('Cleaning database (dropping all tables, indexes, types, extensions) and creating schema...');
    try {
      await client.query(sql);
    } catch (err) {
      logger.warn(`Full schema drop failed (${err.message}); dropping objects individually`);
      await dropObjectsIndividually(client);
      const withoutSchemaDrop = sql
        .split('\n')
        .filter((l) => !/^(DROP SCHEMA|CREATE SCHEMA|GRANT ALL ON SCHEMA)/i.test(l.trim()))
        .join('\n');
...
export async function seedLookups() {
  ...
  await query(SYSTEM_SQL.SEED_APPOINTMENT_STATUSES, [statuses, statuses.map(cap)]);
  await query(SYSTEM_SQL.SEED_INVITATION_STATUSES, [invitation, invitation.map(cap)]);
  await query(SYSTEM_SQL.SEED_FOOD_TIMINGS, [food, food.map((f) => FOOD_TIMING_LABEL[f])]);
}

export async function ensureAdmin() {
  const { rows } = await query(COMMON_SQL.USER_FIND_BY_EMAIL, [env.admin.email]);
  if (rows.length) return rows[0];
  const hash = await hashPassword(env.admin.password);
  const admin = await withTransaction(async (client) => {
    const { rows: created } = await client.query(COMMON_SQL.USER_INSERT, [env.admin.name, env.admin.email, hash, ROLES.ADMIN]);
...
export async function bootstrapDatabase({ force = false } = {}) {
  ...
  if (!initialised) {
    await resetSchema();
    await seedLookups();
    await query(SYSTEM_SQL.META_UPSERT, [META_KEYS.SCHEMA_VERSION, SCHEMA_VERSION]);
  } else {
