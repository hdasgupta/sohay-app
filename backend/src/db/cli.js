...
import { ensureMedicines, medicineCount } from '../services/medicine.service.js';
...
try {
  if (cmd === 'reset') {
    ...
    logger.info(r.freshlyCreated ? 'Database created' : 'Database already initialised');
  } else if (cmd === 'medicines') {
    await ensureMedicines();
  } else if (cmd === 'status') {
    const ready = await isInitialised();
    console.log(JSON.stringify({ initialised: ready, medicines: ready ? await medicineCount() : 0 }));
  } else if (cmd === 'cleanup-test-data') {
    const pattern = `%@${process.argv[3] || 'autotest.wbfmh.local'}`;
    const { rows } = await pool.query(SYSTEM_SQL.TEST_PRESCRIPTION_KEYS, [pattern]);
    const keys = [...new Set(rows.map((r) => r.pdf_key))];
    for (const key of keys) {
      try { await deletePrescription(key); } catch (e) { logger.warn(`Could not delete ${key}: ${e.message}`); }
    }
    const users = await pool.query(SYSTEM_SQL.TEST_USERS_DELETE, [pattern]);
    await pool.query(SYSTEM_SQL.TEST_OTPS_DELETE, [pattern]);
    logger.info(`Test cleanup: ${users.rowCount} user(s), ${keys.length} prescription file(s) removed`);
  } else {
