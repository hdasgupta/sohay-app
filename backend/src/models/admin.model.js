...
export const insertAdmin = (userId, client) =>
  client.query(ADMIN_SQL.ADMIN_INSERT, [userId]);
...
export const updateUserPassword = (client, userId, hash) =>
  client.query(ADMIN_SQL.USER_UPDATE_PASSWORD_BY_ID, [userId, hash]);
...
export const listDoctors = async () =>
  (await query(ADMIN_SQL.DOCTORS_LIST_ALL)).rows;
export const listPatients = async () =>
  (await query(ADMIN_SQL.PATIENTS_LIST_ALL)).rows;
export const setUserDisabled = async (userId, disabled, role) =>
  (await query(ADMIN_SQL.USER_SET_DISABLED, [userId, disabled, role]))
    .rows[0] || null;

export const upcomingForPatientDoctor = async (
  patientId,
  doctorId,
  statuses,
  today,
  ...
  (
    await query(ADMIN_SQL.UPCOMING_FOR_PATIENT_DOCTOR, [
      patientId,
      doctorId,
      statuses,
      today,
...
export const rescheduleAppointment = async (
  client,
  { id, date, startTime, endTime, status, allowedStatuses },
) =>
  (
    await client.query(ADMIN_SQL.APPOINTMENT_RESCHEDULE, [
      id,
      date,
      startTime,
      endTime,
