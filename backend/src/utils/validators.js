...
export const cleanString = (v, max = 255) => (typeof v === 'string' ? v.trim().slice(0, max) : '');
...
export function requireFields(body, fields) {
  ...
  if (missing.length) throw AppError.badRequest(`Missing required field(s): ${missing.join(', ')}`, { missing });
}

export function assertEmail(email) {
  if (!EMAIL_RE.test(email)) throw AppError.badRequest('Please enter a valid email address');
}

export function assertPassword(password, confirmPassword) {
  const errs = passwordErrors(password);
  if (errs.length) throw AppError.badRequest(`Password must contain ${errs.join(', ')}`, { password: errs });
...
export function assertSex(sex) {
  if (!SEXES.includes(sex)) throw AppError.badRequest(`Sex must be one of ${SEXES.join(', ')}`);
}

export function assertDob(dob) {
  if (!isValidIsoDate(dob)) throw AppError.badRequest('Date of birth must be a valid date (YYYY-MM-DD)');
  if (dob < '1900-01-01' || dob > todayInKolkata()) throw AppError.badRequest('Date of birth must be between 1900 and today');
}
...
export function toId(value, name = 'id') {
  ...
  if (!Number.isInteger(n) || n <= 0) throw AppError.badRequest(`Invalid ${name}`);
  return n;
}
