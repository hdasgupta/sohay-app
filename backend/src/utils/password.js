...
export const PASSWORD_RULES = [
  { key: 'upper', test: (p) => /[A-Z]/.test(p), message: 'at least one upper case letter' },
  { key: 'lower', test: (p) => /[a-z]/.test(p), message: 'at least one lower case letter' },
  { key: 'digit', test: (p) => /\d/.test(p), message: 'at least one digit' },
  { key: 'special', test: (p) => /[^A-Za-z0-9\s]/.test(p), message: 'at least one special character' },
  { key: 'length', test: (p) => p.length >= 8, message: 'at least 8 characters' },
