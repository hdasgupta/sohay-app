...
export function validateAvailability(list) {
  if (!Array.isArray(list) || list.length === 0) {
    throw AppError.badRequest(
      "A doctor must have at least one time slot in the week",
    );
  }
  const byDay = new Map();
  for (const item of list) {
    ...
    if (!Number.isInteger(weekday) || weekday < 0 || weekday > 6)
      throw AppError.badRequest("Invalid weekday in availability");
    const re = /^([01]\d|2[0-4]):(00|30)$/;
    if (!re.test(startTime || "") || !re.test(endTime || ""))
      throw AppError.badRequest(
        "Time slots must be on 30 minute boundaries (HH:MM)",
      );
    const s = timeToMinutes(startTime);
    const e = timeToMinutes(endTime);
    if (s >= 1440 || e > 1440 || e <= s)
  ...
  for (const [weekday, ranges] of byDay) {
    ...
    for (let i = 1; i < ranges.length; i += 1) {
      if (ranges[i].s < ranges[i - 1].e) {
        throw AppError.badRequest(`Time slots overlap on weekday ${weekday}`, {
          weekday,
        });
      }
    }
