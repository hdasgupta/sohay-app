...
let server; let base; let patient; let doctorId; let doctorToken;
...
async function load(name, makeRequest, total = REQUESTS) {
  ...
  await Promise.all(Array.from({ length: CONCURRENCY }, async () => {
    while (next < total) { next += 1; results.push(await makeRequest()); }
  }));
  ...
  const summary = { name, requests: results.length, rps: Math.round(results.length / secs), p50: pct(50).toFixed(1), p95: pct(95).toFixed(1), p99: pct(99).toFixed(1), errors };
...
test('captcha generation under load', async () => {
  const r = await load('GET /api/captcha', () => call('/api/captcha'), Math.round(REQUESTS / 4));
  assert.equal(r.errors, 0);
  assert.ok(r.p95n < P95_BUDGET_MS * 3, `p95 ${r.p95} ms`);
});

test('authenticated doctor listing for patients', async () => {
  const r = await load('GET /api/patient/doctors', () => call('/api/patient/doctors', { token: patient.token }));
  assert.equal(r.errors, 0);
  assert.ok(r.p95n < P95_BUDGET_MS, `p95 ${r.p95} ms`);
});
...
test('slot availability computation', async () => {
  ...
  const r = await load('GET /api/patient/slots', () => call(`/api/patient/slots?doctorId=${doctorId}&date=${date}`, { token: patient.token }));
  assert.equal(r.errors, 0);
  assert.ok(r.p95n < P95_BUDGET_MS, `p95 ${r.p95} ms`);
});

test('medicine search', async () => {
  const r = await load('GET /api/doctor/medicines', () => call('/api/doctor/medicines?q=para', { token: doctorToken }));
  assert.equal(r.errors, 0);
  assert.ok(r.p95n < P95_BUDGET_MS, `p95 ${r.p95} ms`);
});
...
test('concurrent booking storm: exactly one booking per slot wins', async () => {
  ...
  const request = H.api();
  const patients = [];
  for (let i = 0; i < 10; i += 1) patients.push(await H.registerPatient(request, { name: `Storm ${i}` }));
  const results = await Promise.all(patients.map((p) => call('/api/patient/appointments', { method: 'POST', token: p.token, body: { doctorId, date, startTime: '10:00' } })));
