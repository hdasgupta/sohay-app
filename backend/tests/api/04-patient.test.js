...
describe("patient: doctors and slots", () => {
  ...
  test("slots are 30 minute divisions of availability", async () => {
    const r = await request
      .get(`/api/patient/slots?doctorId=${docA.id}&date=${tomorrow()}`)
      .set(auth(p1.token))
      .expect(200);
  ...
  test("no slots on a weekday the doctor does not work", async () => {
    const d = await createDoctor(request, admin, {
      availability: [
        {
    ...
    const r1 = await request
      .get(`/api/patient/slots?doctorId=${d.id}&date=${tomorrow()}`)
      .set(auth(p1.token))
      .expect(200);
    assert.deepEqual(r1.body.data, ["10:00", "10:30"]);
    const r2 = await request
      .get(`/api/patient/slots?doctorId=${d.id}&date=${addDays(tomorrow(), 1)}`)
      .set(auth(p1.token))
      .expect(200);
  ...
  test("past dates have no slots; invalid date rejected", async () => {
    const r = await request
      .get(
        `/api/patient/slots?doctorId=${docA.id}&date=${addDays(todayInKolkata(), -1)}`,
      )
      .set(auth(p1.token))
      .expect(200);
    assert.deepEqual(r.body.data, []);
    await request
      .get(`/api/patient/slots?doctorId=${docA.id}&date=2026-13-45`)
      .set(auth(p1.token))
      .expect(400);
  });
...
describe("patient: booking and conflicts", () => {
  ...
  test("book appointment -> email with in-app 8x8 meeting link", async () => {
    ...
    const r = await request
      ...
      .set(auth(p1.token))
      .send({ doctorId: docA.id, date: tomorrow(), startTime: "10:00" })
      .expect(201);
  ...
  test("same patient + same slot with another doctor -> 409", async () => {
    const r = await request
      .post("/api/patient/appointments")
      .set(auth(p1.token))
      .send({ doctorId: docB.id, date: tomorrow(), startTime: "10:00" });
    assert.equal(r.status, 409);
    const s = await request
      .get(`/api/patient/slots?doctorId=${docB.id}&date=${tomorrow()}`)
      .set(auth(p1.token))
      .expect(200);
    ...
    const results = await Promise.all(
      [p1, p2].map((p) =>
        request
          .post("/api/patient/appointments")
          .set(auth(p.token))
          .send({ doctorId: docB.id, date: tomorrow(), startTime: "15:00" }),
      ),
    );
    ...
    assert.equal(
      (
        await request
          ...
          .send({
            doctorId: docA.id,
            date: addDays(todayInKolkata(), 200),
            startTime: "10:00",
          })
      ).status,
      400,
    ...
    assert.equal(
      (
        await request
          .post("/api/patient/appointments")
          .set(auth(p1.token))
          .send({ doctorId: docA.id, date: tomorrow(), startTime: "10:15" })
      ).status,
      409,
    );
    assert.equal(
  ...
  test("cancel upcoming appointment frees the slot", async () => {
    await request
      .patch(`/api/patient/appointments/${apptId}/cancel`)
      .set(auth(p1.token))
      .expect(200);
    const s = await request
      .get(`/api/patient/slots?doctorId=${docA.id}&date=${tomorrow()}`)
  ...
  test("other patient cannot cancel", async () => {
    const r = await request
      .post("/api/patient/appointments")
      .set(auth(p1.token))
      .send({ doctorId: docA.id, date: tomorrow(), startTime: "12:00" })
      .expect(201);
    await request
      .patch(`/api/patient/appointments/${r.body.data.id}/cancel`)
      .set(auth(p2.token))
      .expect(403);
  });
});
...
describe("patient: family management", () => {
  ...
  test("cannot invite before creating a family", async () => {
    await request
      .post("/api/patient/family/invitations")
      .set(auth(owner.token))
  ...
  test("invite validations", async () => {
    ...
    await request
      .post("/api/patient/family/invitations")
      .set(auth(owner.token))
      .send({ email: "nobody@autotest.wbfmh.local" })
      .expect(404);
  });
  test("invite -> notification email -> invitee sees it", async () => {
    ...
    assert.ok(
      getOutbox().some(
        (m) => m.to === member.email && /invited you/.test(m.subject),
      ),
    );
    await request
      .post("/api/patient/family/invitations")
  ...
  test("reject -> inviter notified", async () => {
    clearOutbox();
    await request
      .post(`/api/patient/family/invitations/${invitationId}/respond`)
      .set(auth(member.token))
      .send({ action: "reject" })
      .expect(200);
    assert.ok(
      getOutbox().some(
        (m) => m.to === owner.email && /declined/.test(m.subject),
      ),
    );
    await request
      .post(`/api/patient/family/invitations/${invitationId}/respond`)
  ...
  test("accept -> joins family -> inviter notified", async () => {
    ...
    assert.ok(
      getOutbox().some(
        (m) => m.to === owner.email && /accepted/.test(m.subject),
      ),
    );
    const r = await request
      .get("/api/patient/members")
  ...
  test("withdraw pending invitation", async () => {
    ...
    const r = await request
      ...
      .expect(201);
    await request
      .delete(`/api/patient/family/invitations/${r.body.data.id}`)
      .set(auth(owner.token))
      .expect(200);
    const f = await request
      .get("/api/patient/family")
      .set(auth(extra.token))
      .expect(200);
    assert.equal(f.body.data.receivedInvitations.length, 0);
  });
  test("leave family", async () => {
    await request
      .post("/api/patient/family/leave")
      .set(auth(member.token))
      .expect(200);
    const r = await request
      .get("/api/patient/members")
