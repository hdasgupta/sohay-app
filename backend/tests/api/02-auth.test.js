...
describe("captcha", () => {
  ...
  test("captcha is single use", async () => {
    ...
    await request
      .post("/api/auth/login")
      .send({ email: env.admin.email, password: env.admin.password, ...c })
      .expect(200);
  ...
  test("wrong captcha rejected", async () => {
    const c = await captcha(request);
    const r = await request.post("/api/auth/login").send({
...
describe("login with automatic role detection", () => {
  test("seeded admin can login and role is detected", async () => {
    ...
    assert.ok(
      new Date(r.body.data.expiresAt) > new Date(Date.now() + 29 * 86400000),
    );
  });
  test("email is case insensitive", async () => {
    const r = await login(
      request,
      env.admin.email.toUpperCase(),
      env.admin.password,
    );
    assert.equal(r.status, 200);
  });
...
describe("patient registration with email OTP", () => {
  ...
  test("wrong OTP rejected", async () => {
    const r = await request.post("/api/auth/otp/verify").send({
      email,
      purpose: "register",
      otp: "000000" === otpFromOutbox(email) ? "111111" : "000000",
    });
    assert.equal(r.status, 400);
  });
  test("correct OTP returns verification token", async () => {
    const r = await request
      ...
      .send({ email, purpose: "register", otp: otpFromOutbox(email) })
      .expect(200);
  ...
  test("weak password rejected", async () => {
    ...
    const r = await request
      ...
      .send({ ...base(), password: "weak", confirmPassword: "weak", ...c });
    assert.equal(r.status, 400);
  ...
  test("password mismatch rejected", async () => {
    ...
    const r = await request
      .post("/api/auth/register")
      .send({ ...base(), confirmPassword: "Other@12345", ...c });
    assert.equal(r.status, 400);
  });
  ...
  test("verification token for another email rejected", async () => {
    ...
    const r = await request
      .post("/api/auth/register")
      .send({ ...base(), email: uniqueEmail("other"), ...c });
    assert.equal(r.status, 400);
  ...
  test("admin email also blocks patient registration (unique across all user types)", async () => {
    const r = await request
      .post("/api/auth/otp/send")
      .send({ email: env.admin.email, purpose: "register" });
    assert.equal(r.status, 409);
  });
});
...
describe("reset password", () => {
  test("unknown email -> error", async () => {
    const r = await request
      .post("/api/auth/otp/send")
      .send({ email: uniqueEmail("ghost"), purpose: "reset_password" });
  ...
  test("reset with verified OTP and login with new password", async () => {
    ...
    await request
      .post("/api/auth/otp/send")
      .send({ email: p.email, purpose: "reset_password" })
      .expect(200);
  ...
  test("register-purpose token cannot reset a password", async () => {
    const p = await registerPatient(request);
    const c = await captcha(request);
    const r = await request.post("/api/auth/reset-password").send({
      email: p.email,
      password: "NewPass@123",
      confirmPassword: "NewPass@123",
      verificationToken: signVerificationTokenForTest(p.email, "register"),
      ...c,
    });
    assert.equal(r.status, 400);
  });
