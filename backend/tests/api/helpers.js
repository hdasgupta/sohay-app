...
const { peekCaptchaAnswer } =
  await import("../../src/services/captcha.service.js");
const { getOutbox, clearOutbox } =
  await import("../../src/services/email.service.js");
const { signVerificationTokenForTest } =
  await import("../../src/services/otp.service.js");
const { todayInKolkata, addDays, weekdayOf } =
...
export const uniqueEmail = (tag = "u") =>
  `${tag}.${Date.now()}.${Math.floor(Math.random() * 1e6)}@${TEST_DOMAIN}`;

export async function captcha(request) {
  const res = await request.get("/api/captcha").expect(200);
  return {
    captchaId: res.body.data.captchaId,
    captchaText: peekCaptchaAnswer(res.body.data.captchaId),
  };
}
...
export async function tokenFor(request, email, password = PASSWORD) {
  ...
  if (res.status !== 200)
    throw new Error(
      `login failed for ${email}: ${res.status} ${res.body.message}`,
    );
...
export async function registerPatient(request, overrides = {}) {
  ...
  if (res.status !== 201)
    throw new Error(`register failed: ${res.status} ${res.body.message}`);
...
export async function createDoctor(request, adminTok, overrides = {}) {
  ...
  const res = await request
    ...
    .send({
      ...
      password: PASSWORD,
      confirmPassword: PASSWORD,
      availability: overrides.availability || fullWeek(),
      ...overrides,
    });
  if (res.status !== 201)
    throw new Error(`doctor create failed: ${res.status} ${res.body.message}`);
...
export async function cleanup() {
  ...
  await pool.query("DELETE FROM email_otps WHERE email LIKE $1", [
    `%@${TEST_DOMAIN}`,
  ]);
