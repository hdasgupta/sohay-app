...
export const listPatients = 'SELECT u.id,u.name,u.email FROM users u JOIN patients p ON p.user_id=u.id WHERE u.is_disabled=$1 ORDER BY u.name ASC';
export const emailConflict = 'SELECT id FROM users WHERE email=$1 AND id<>$2 LIMIT $3';
...
export const createUser = 'INSERT INTO users(name,email,password_hash,is_disabled) VALUES($1,$2,$3,$4) RETURNING id';
export const createDoctorRole = 'INSERT INTO doctors(user_id,sex,speciality) VALUES($1,$2,$3)';
export const createAvailability = 'INSERT INTO doctor_availability(doctor_id,weekday,start_time,end_time) VALUES($1,$2,$3,$4)';
export const deleteAvailability = 'DELETE FROM doctor_availability WHERE doctor_id=$1';
export const getDoctor = 'SELECT u.id,u.name,u.email,u.is_disabled,d.sex,d.speciality FROM users u JOIN doctors d ON d.user_id=u.id WHERE u.id=$1 LIMIT $2';
export const getAvailability = 'SELECT weekday,start_time,end_time FROM doctor_availability WHERE doctor_id=$1 ORDER BY weekday,start_time';
export const updateDoctor = 'UPDATE users SET name=$1,email=$2,password_hash=COALESCE($3,password_hash),updated_at=NOW() WHERE id=$4';
export const updateDoctorRole = 'UPDATE doctors SET sex=$1,speciality=$2 WHERE user_id=$3';
export const setDisabled = 'UPDATE users SET is_disabled=$1,updated_at=NOW() WHERE id=$2 AND EXISTS(SELECT $3 FROM doctors WHERE user_id=users.id)';
...
export const rescheduleAppointment = 'UPDATE appointments SET appointment_date=$1,start_time=$2,end_time=$3,active_doctor_slot_key=$4,active_patient_slot_key=$5,status=$6,updated_at=NOW() WHERE id=$7 AND status IN ($8,$9)';
