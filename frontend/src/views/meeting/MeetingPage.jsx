    ...
    scriptPromises.set(
      ...
      new Promise((resolve, reject) => {
        ...
        s.onerror = () => {
          ...
          reject(new Error("Could not load the video meeting library"));
        };
...
export default function MeetingPage() {
    ...
    (async () => {
      try {
        ...
        api.addListener("recordingStatusChanged", (e) => {
          ...
          else if (e?.on && data.autoRecord)
            notify.info("This consultation is being recorded");
        });
  ...
  return (
    <div className="meeting">
        ...
        <div>
          ...
          {a && (
            <p className="muted small">
              ...
              {formatDate(a.date)} · {slotLabel(a.date, a.startTime, a.endTime)}
            </p>
          )}
        </div>
        <div className="row">
          {recording && (
            <span className="meet-rec" aria-live="polite">
              <span className="meet-rec-dot" /> Recording
            </span>
          )}
          {user.role === "doctor" && (
            <button
      ...
      {status === "error" ? (
        <div className="empty-state meet-error">
          <strong>Unable to open the consultation room</strong>
          {errorText}
        </div>
      ) : (
        <div
