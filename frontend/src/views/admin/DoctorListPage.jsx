...
export default function DoctorListPage() {
  ...
  const load = useCallback(async () => {
    try {
      setDoctors((await listDoctors()).data);
    } catch {
      setDoctors([]);
    }
  }, []);
  ...
  const toggle = async (doc) => {
    ...
    try {
      ...
      setDoctors((list) =>
        list.map((d) =>
          d.id === doc.id ? { ...d, isDisabled: !doc.isDisabled } : d,
        ),
      );
    } catch {
      /* shown */
  ...
  const renderDoctor = (d) => (
    <article
      className={`doc-card ${d.isDisabled ? "doc-disabled" : ""}`}
    ...
    >
      <header className="doc-head">
        ...
        <span
          className={`badge ${d.isDisabled ? "badge-disabled" : "badge-enabled"}`}
        >
          {d.isDisabled ? "Disabled" : "Active"}
        </span>
      </header>
      <ul className="doc-meta">
        ...
        <li>
          <span className="faint">Sex</span>
          {d.sex}
        </li>
        <li>
          <span className="faint">Signature</span>
          {d.hasSignature ? "Added" : "Not added yet"}
        </li>
      </ul>
      <div className="doc-week" aria-label="Weekly availability">
        {groupAvailability(d.availability).map(([day, slots]) => (
          <div
            key={day}
            className={`doc-day ${slots.length ? "doc-day-on" : ""}`}
          >
            <span className="doc-day-name">{WEEKDAYS_SHORT[day]}</span>
            <span className="doc-day-slots">
              {slots.length
                ? slots
                    .map(
                      (s) =>
    ...
    <div className="doctor-list">
      <div className="page-title">
        <div>
          <h1>Doctors</h1>
          <p>
            {doctors ? `${doctors.length} doctor(s) registered` : "Loading..."}.
            Disabled doctors cannot login and are hidden from patients.
          </p>
        </div>
        <button
          type="button"
      ...
      {doctors && (
        <Pagination
          ...
          searchText={(d) =>
            `${d.name} ${d.email} ${d.speciality} ${d.sex} ${d.isDisabled ? "disabled" : "active"}`
          }
