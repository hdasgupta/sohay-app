...
export default function RescheduleAppointment() {
  ...
  useEffect(() => {
    Promise.all([api.get("/admin/patients"), api.get("/admin/doctors")])
      .then(([p, d]) => {
        setPatients(p.data.items);
        setDoctors(d.data.items.filter((x) => !x.is_disabled));
      })
  ...
  useEffect(() => {
    ...
    api
      .get(
        `/admin/appointments/upcoming?patientId=${patientId}&doctorId=${doctorId}`,
      )
      .then((r) => {
        setApt(r.data.appointment);
        if (r.data.appointment)
          setDate(String(r.data.appointment.appointment_date).slice(0, 10));
      })
      .catch((e) => notify("error", getErrorMessage(e)));
  }, [patientId, doctorId]);
  useEffect(() => {
    ...
    api
      .get(
        `/admin/appointments/availability?doctorId=${doctorId}&patientId=${patientId}&date=${date}`,
      )
      .then((r) => {
  ...
  const save = async () => {
    if (!apt) return notify("warning", "No upcoming appointment is available.");
    try {
      await api.patch("/admin/appointments/reschedule", {
    ...
    <div className="panel">
      ...
      <div className="reschedule-grid">
        <div className="field">
          ...
          <Dropdown
            options={patients}
            value={patientId}
            onOptionSelected={(x) => setPatientId(x.id)}
      ...
      {apt && (
        <div className="reschedule-grid">
          ...
          <div className="field">
            <label>30-minute time slot</label>
            <Dropdown
              options={slots.map((v) => ({ label: v, value: v }))}
              value={start}
              onOptionSelected={(x) => setStart(x.value)}
