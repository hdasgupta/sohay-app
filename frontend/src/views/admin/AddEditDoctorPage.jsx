...
export default function AddEditDoctorPage() {
  ...
  const submit = async (e) => {
    ...
    if (form.name.trim().length < 2) {
      notify.warning("Please enter the doctor name");
      return;
    }
    if (!form.sex) {
      notify.warning("Please select sex");
      return;
    }
    if (form.speciality.trim().length < 2) {
      notify.warning("Please enter the speciality");
      return;
    }
    ...
    if (wantsPassword && !isPasswordValid(form.password)) {
      notify.warning("Password does not satisfy all the rules");
      return;
    }
    if (wantsPassword && form.password !== form.confirmPassword) {
    ...
    <div className="add-doctor">
      <div className="page-title">
        <div>
          ...
          <p>
            {isEdit
              ? "Update details and weekly availability. Leave the password empty to keep it unchanged."
              : "Create a doctor account and publish their weekly consultation hours."}
          </p>
        </div>
        {isEdit && (
          <button
      ...
      <form onSubmit={submit} noValidate className="ad-form">
        <section className="card ad-section">
          ...
          <div className="grid grid-2">
            <div className="field">
              <label htmlFor="doc-name" className="required">
                Doctor name
              </label>
              <input
                id="doc-name"
            ...
            />
            <div className="field">
              <label htmlFor="doc-spec" className="required">
                Speciality
              </label>
              <input
                id="doc-spec"
            ...
            <div className="field">
              <label htmlFor="doc-email" className="required">
                Email address
              </label>
              <input
                id="doc-email"
        ...
        <section className="card ad-section">
          <h2 className="ad-section-title">
            <span>2</span> Login password{" "}
            {isEdit && <em className="ad-optional">optional</em>}
          </h2>
          <div className="grid grid-2">
            <Password
              value={form.password}
        ...
        <section className="card ad-section">
          <h2 className="ad-section-title">
            <span>3</span> Weekly time slots
          </h2>
          <p className="muted small">
            Toggle the days the doctor consults and add one or more
            non-overlapping time ranges. Patients book 30 minute slots inside
            these ranges.
          </p>
          <AvailabilityEditor
        ...
        <section className="card ad-section">
          <h2 className="ad-section-title">
            <span>4</span> Signature{" "}
            <em className="ad-optional">
              optional - the doctor can also add it later
            </em>
          </h2>
