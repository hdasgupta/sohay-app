...
export default function LoginPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const target = isPathAllowed(data.user.role, wanted?.split("?")[0])
        ? wanted
        : defaultPathFor(data.user.role);
  ...
  return (
    <AuthLayout
      title="Welcome back"
      subtitle="Sign in to book, attend or manage consultations. Admin, doctors and patients all sign in here."
    >
      {from && (
        <div className="login-deeplink">
          Please login to continue to <strong>{from}</strong>
        </div>
      )}
