...
/** 00:00 ... 23:30 */
export const START_TIMES = Array.from({ length: 48 }, (_, i) => toHHMM(i * SLOT_MINUTES));
/** 00:30 ... 24:00 */
export const END_TIMES = Array.from({ length: 48 }, (_, i) => toHHMM((i + 1) * SLOT_MINUTES));

/** Start options for a slot row: times not inside any other slot of the day */
export function startOptionsFor(daySlots, index) {
  const others = daySlots.filter((_, i) => i !== index && daySlots[i].startTime && daySlots[i].endTime);
  return START_TIMES.filter((t) => {
    const m = toMinutes(t);
    return !others.some((o) => m >= toMinutes(o.startTime) && m < toMinutes(o.endTime));
  });
}
...
export function availabilityError(availability) {
  ...
  if (complete.length === 0) return 'A doctor must have at least one time slot in the week';
  if (complete.length !== availability.length) return 'Please complete or remove the empty time slot rows';
  const byDay = {};
  for (const a of complete) {
    if (toMinutes(a.endTime) <= toMinutes(a.startTime)) return 'End time must be after start time';
    (byDay[a.weekday] ||= []).push(a);
  }
  for (const list of Object.values(byDay)) {
    const sorted = [...list].sort((x, y) => toMinutes(x.startTime) - toMinutes(y.startTime));
    for (let i = 1; i < sorted.length; i += 1) {
      if (toMinutes(sorted[i].startTime) < toMinutes(sorted[i - 1].endTime)) return 'Time slots of a day must not overlap';
    }
  }
  return null;
}
...
export const totalHours = (availability) =>
  availability.filter((a) => a.startTime && a.endTime).reduce((s, a) => s + (toMinutes(a.endTime) - toMinutes(a.startTime)), 0) / 60;
