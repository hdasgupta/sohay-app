...
let seq = 0;
export const nextId = () => { seq += 1; return seq; };

/** Imperative helpers */
export const notify = {
  info: (text, duration) => messageBus.emit({ type: 'info', text, duration }),
  success: (text, duration) => messageBus.emit({ type: 'success', text, duration }),
  warning: (text, duration) => messageBus.emit({ type: 'warning', text, duration }),
