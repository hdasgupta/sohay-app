...
export const browserTimeZone = () =>
  Intl.DateTimeFormat().resolvedOptions().timeZone || CLINIC_TZ;
export const isClinicTimeZone = () => {
  const offsetNow = new Date().toLocaleString("en-US", {
    timeZone: browserTimeZone(),
    timeZoneName: "shortOffset",
  });
  const offsetClinic = new Date().toLocaleString("en-US", {
    timeZone: CLINIC_TZ,
    timeZoneName: "shortOffset",
  });
  return (
    browserTimeZone() === CLINIC_TZ ||
    offsetNow.split(" ").pop() === offsetClinic.split(" ").pop()
  );
};
...
export function formatDate(iso) {
  if (!iso) return "";
  const { y, m, d } = parseIso(iso);
  return new Date(Date.UTC(y, m, d)).toLocaleDateString("en-IN", {
    weekday: "short",
    day: "2-digit",
    month: "short",
    year: "numeric",
...
export const clinicToDate = (iso, hhmm) =>
  new Date(`${iso}T${hhmm === "24:00" ? "23:59" : hhmm}:00+05:30`);
...
export const isFuture = (iso, hhmm, now = new Date()) =>
  clinicToDate(iso, hhmm).getTime() > now.getTime();
