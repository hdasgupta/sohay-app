...
const sortRows = (rows) =>
  [...rows].sort(
    (a, b) =>
      a.weekday - b.weekday ||
      toMinutes(a.startTime || "99:99") - toMinutes(b.startTime || "99:99"),
  );
...
export default function AvailabilityEditor({ value = [], onChange, error }) {
  const dayRows = (day) =>
    value.map((r, i) => ({ ...r, gi: i })).filter((r) => r.weekday === day);
  ...
  const addSlot = (day) => {
    ...
    const lastEnd = busy.length
      ? Math.max(...busy.map((r) => toMinutes(r.endTime)))
      : toMinutes("09:00");
    const start =
      candidateStarts.find((t) => toMinutes(t) >= lastEnd) ||
      candidateStarts[0];
    if (!start) {
      notify.warning(`${WEEKDAYS[day]} is already fully covered`);
      return;
    }
    const probe = [...busy, { weekday: day, startTime: start, endTime: null }];
    const ends = endOptionsFor(probe, probe.length - 1);
    const end =
      ends.find((t) => toMinutes(t) >= toMinutes(start) + 60) ||
      ends[ends.length - 1];
  ...
  const update = (gi, patch) => {
    ...
    const same = next
      .map((r, i) => ({ ...r, gi: i }))
      .filter((r) => r.weekday === row.weekday);
  ...
  return (
    <div className="avail-editor">
      <div className="ae-summary">
        <span className="chip">
          {new Set(value.map((r) => r.weekday)).size} day(s)
        </span>
        <span className="chip">{value.length} slot(s)</span>
        <span className="chip">
          Total {hours % 1 === 0 ? hours : hours.toFixed(1)} h / week
        </span>
      </div>
      {error && (
        <p className="error-text ae-error" role="alert">
          {error}
        </p>
      )}
          ...
          return (
            ...
            >
              <header className="ae-day-head">
                <label className="ae-switch">
                  <input
                    ...
                    aria-label={`${WEEKDAYS[day]} available`}
                  />
                ...
                {on && (
                  <button
                    ...
                    title="Copy these slots to every day"
                  >
                    Copy to all
                  </button>
                )}
                ...
                return (
                  <div className="ae-slot" key={`${day}-${r.gi}`}>
                    <Dropdown
                      ...
                      options={starts}
                      keyProcessor={(t) => t}
                      labelProcessor={formatTime}
                      selected={r.startTime}
                      placeholder="Start"
                      onOptionSelected={(t) => update(r.gi, { startTime: t })}
                      id={`start-${day}-${localIdx}`}
                    />
                    ...
                    <Dropdown
                      ...
                      selected={r.endTime}
                      placeholder="End"
                      onOptionSelected={(t) => update(r.gi, { endTime: t })}
                      id={`end-${day}-${localIdx}`}
                    ...
                    <button
                      ...
                      className="ae-remove"
                      onClick={() => remove(r.gi)}
                      aria-label={`Remove ${WEEKDAYS[day]} slot ${localIdx + 1}`}
                    >
                      ×
                    </button>
                  </div>
              ...
              {on && (
                <button
                  type="button"
                  className="btn btn-ghost btn-sm ae-add"
                  onClick={() => addSlot(day)}
                >
                  + Add slot
                </button>
