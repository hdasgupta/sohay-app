 ...
 */
export default function Captcha({ onChange, refreshKey = 0, label = 'Captcha' }) {
  const autoId = useId();
  ...
  return (
    <div className="field captcha">
      <label htmlFor={`cap-${autoId}`} className="required">{label}</label>
      <div className="cap-row">
        <div className="cap-image" aria-label="Captcha image" role="img">
          <canvas ref={canvasRef} width="220" height="70" data-captcha-id={captchaId || ''} />
          {failed && <span className="cap-failed">Could not load</span>}
        </div>
        <button type="button" className="icon-btn cap-refresh" onClick={load} aria-label="Load a new captcha" title="New captcha">
      ...
      <input id={`cap-${autoId}`} className="input cap-input" value={text} onChange={(e) => update(e.target.value)} placeholder="Type the characters shown above" autoComplete="off" autoCapitalize="characters" spellCheck="false" required />
      <span className="hint">Letters are not case sensitive. Click refresh if it is hard to read.</span>
    </div>
  );
}
