...
export default function AnimatedRoutes({ children, className = '' }) {
  ...
  const [layers, setLayers] = useState([{ key: location.pathname, location, phase: 'enter' }]);
  ...
  useEffect(() => {
    setLayers((prev) => {
      ...
      const exiting = prev.filter((l) => l.key !== location.pathname).slice(-1).map((l) => ({ ...l, phase: 'exit' }));
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setLayers((prev) => prev.filter((l) => l.phase !== 'exit')), EXIT_MS + 150);
    return () => clearTimeout(t);
  }, [hasExit, layers]);

  const onAnimationEnd = (key, phase) => (e) => {
    if (e.target !== e.currentTarget) return;
    if (phase === 'exit') setLayers((prev) => prev.filter((l) => l.key !== key || l.phase !== 'exit'));
    else setLayers((prev) => prev.map((l) => (l.key === key ? { ...l, phase: 'idle' } : l)));
  };
