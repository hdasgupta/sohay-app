...
client.interceptors.request.use((config) => {
  ...
  if (!config.silentLoader) {
    ...
    loaderBus.emit({ type: 'start', id: config.loaderId, message: config.loaderMessage || 'Please wait...' });
  }
  logger.info(`API -> ${config.method?.toUpperCase()} ${config.url}`);
  return config;
});

const stopLoader = (config) => { if (config?.loaderId) loaderBus.emit({ type: 'stop', id: config.loaderId }); };
...
client.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (!message) {
      if (error.code === 'ECONNABORTED') message = 'The server took too long to respond. Please try again';
      else if (!error.response) message = 'Unable to reach the server. Please check your internet connection (the server may be waking up, retry in a few seconds)';
      else message = `Request failed (${status})`;
    }
    logger.error(`API x ${status || 'network'} ${error.config?.url}: ${message}`);
    const isAuthCall = /\/auth\/(login|otp|register|reset-password)/.test(error.config?.url || '');
...
export const unwrap = (res) => ({ data: res.data?.data, message: res.data?.message });
