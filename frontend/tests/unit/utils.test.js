...
import { START_TIMES, END_TIMES, startOptionsFor, endOptionsFor, availabilityError, totalHours, toMinutes } from '../../src/utils/timeSlots.js';
...
import { ageFromDob, addDaysIso, weekdayOfIso, daysInMonth, formatTime, todayIso, isFuture, clinicToDate } from '../../src/utils/date.js';
import { isEmail, isPhone, initials } from '../../src/utils/validators.js';
import { saveAuth, loadAuth, clearAuth, rememberRedirect, takeRedirect } from '../../src/utils/authStorage.js';
...
describe('timeSlots', () => {
  ...
  it('end options stop at the next slot start and start options skip busy ranges', () => {
    const day = [{ startTime: '09:00', endTime: '10:00' }, { startTime: '11:00', endTime: '12:00' }];
  ...
  it('validates availability', () => {
    ...
    expect(availabilityError([{ weekday: 1, startTime: '10:00', endTime: '' }])).toMatch(/at least one|complete/);
    expect(availabilityError([{ weekday: 1, startTime: '10:00', endTime: '09:00' }])).toMatch(/after/);
    expect(availabilityError([{ weekday: 1, startTime: '09:00', endTime: '11:00' }, { weekday: 1, startTime: '10:00', endTime: '12:00' }])).toMatch(/overlap/);
    expect(availabilityError([{ weekday: 1, startTime: '09:00', endTime: '11:00' }, { weekday: 2, startTime: '10:00', endTime: '12:00' }])).toBeNull();
    expect(totalHours([{ weekday: 1, startTime: '09:00', endTime: '11:30' }])).toBe(2.5);
  });
});
...
describe('medicine', () => {
  ...
  it('summarises timings', () => {
    expect(timingSummary({ morning: true, night: true })).toBe('Morning · Night');
...
describe('password', () => {
  it('has the six required rules', () => {
    expect(PASSWORD_RULES.map((r) => r.key)).toEqual(['upper', 'lower', 'digit', 'special', 'length', 'space']);
  });
...
describe('date', () => {
  ...
  it('uses Asia/Kolkata for today and for slot instants', () => {
    ...
    expect(clinicToDate('2026-09-22', '10:00').toISOString()).toBe('2026-09-22T04:30:00.000Z');
    expect(isFuture('2026-09-22', '10:00', new Date('2026-09-22T04:00:00Z'))).toBe(true);
  });
});
...
describe('authStorage', () => {
  ...
  it('drops an expired session', () => {
    localStorage.setItem('wbfmh.auth', JSON.stringify({ token: 't', user: {}, expiresAt: Date.now() - 1000 }));
    expect(loadAuth()).toBeNull();
  });
