...
describe('frontend performance', () => {
  it('paginates and searches 10,000 items quickly', async () => {
    const items = Array.from({ length: 10000 }, (_, i) => ({ id: i, name: `Patient ${i}`, email: `p${i}@example.com` }));
    const t0 = performance.now();
    render(<Pagination items={items} itemProcessor={(it) => <span data-testid="row">{it.name}</span>} />);
    ...
    console.log(`render 10k: ${renderMs.toFixed(1)} ms, search: ${searchMs.toFixed(1)} ms`);
  ...
  it('guesses doses for 250,000 medicine names in well under a second', () => {
    const names = ['Crocin 500 Tablet', 'Benadryl Syrup', 'Amoxil 250 Capsule', 'Volini Gel'];
    const t0 = performance.now();
  ...
  it('computes slot options for a full day repeatedly', () => {
    const day = [{ startTime: '00:00', endTime: '06:00' }, { startTime: '08:00', endTime: '12:00' }, { startTime: '14:00', endTime: '' }];
    const t0 = performance.now();
