...
import AnimatedRoutes, { EXIT_MS, Redirect } from '../../src/components/AnimatedRoutes/AnimatedRoutes.jsx';
...
vi.mock('../../src/api/commonApi.js', () => ({
  getCaptcha: vi.fn(() => Promise.resolve({ data: { captchaId: 'cap-123', image: 'data:image/png;base64,AAAA' } })),
}));

describe('DatePicker', () => {
  it('disables weekdays that are not enabled and selects enabled ones', async () => {
    const onSel = vi.fn();
    render(<DatePicker label="Date" value="2026-09-22" startDate="2026-09-01" endDate="2026-09-30" enabledWeekDays={[1, 3]} onDateSelect={onSel} />);
  ...
  it('respects start and end dates', async () => {
    render(<DatePicker label="Date" value="2026-09-15" startDate="2026-09-10" endDate="2026-09-20" onDateSelect={() => {}} />);
    await userEvent.click(screen.getByLabelText('Date'));
    const dlg = screen.getByRole('dialog');
  ...
  it('year selection shows decades first, starting from 1900, then years then months', async () => {
    const onSel = vi.fn();
    render(<DatePicker label="DOB" endDate="2026-09-22" onDateSelect={onSel} />);
    ...
    const decades = [...dlg.querySelectorAll('[data-decade]')].map((b) => Number(b.dataset.decade));
  ...
  it('month view is reachable from the day header', async () => {
    render(<DatePicker label="Date" value="2026-09-15" onDateSelect={() => {}} />);
    await userEvent.click(screen.getByLabelText('Date'));
...
describe('Pagination', () => {
  const items = Array.from({ length: 57 }, (_, i) => ({ id: i + 1, name: `Item ${i + 1}`, city: i % 2 ? 'Kolkata' : 'Howrah' }));
  const renderPg = () => render(<Pagination items={items} itemProcessor={(it) => <span data-testid="row">{it.name}</span>} />);
  ...
  it('shows at most 5 page links', async () => {
    ...
    expect(within(top).getAllByRole('button', { name: /^Page \d+$/ })).toHaveLength(5);
    await userEvent.click(within(top).getByRole('button', { name: 'Last page' }));
    expect(within(top).getAllByRole('button', { name: /^Page \d+$/ }).map((b) => b.textContent)).toEqual(['8', '9', '10', '11', '12']);
    expect(screen.getAllByTestId('row')).toHaveLength(2);
  });
  it('offers 5 / 10 / 20 per page', async () => {
    renderPg();
    const sel = screen.getByTestId('pagination-top').querySelector('select');
    expect([...sel.options].filter((o) => !o.disabled).map((o) => o.value)).toEqual(['5', '10', '20']);
  ...
  it('filters with free text search', async () => {
    ...
    expect(screen.getAllByTestId('row').map((r) => r.textContent)).toEqual(['Item 5', 'Item 50', 'Item 51', 'Item 52', 'Item 53']);
...
describe('AvailabilityEditor', () => {
  function Harness({ initial = [] }) {
    ...
    return <><AvailabilityEditor value={v} onChange={setV} /><output data-testid="val">{JSON.stringify(v)}</output></>;
  }
  ...
  it('enabling a day adds a default slot', async () => {
    ...
    expect(val()).toEqual([{ weekday: 1, startTime: '09:00', endTime: '10:00' }]);
  });
  it('end time options only list times after the start', async () => {
    render(<Harness initial={[{ weekday: 1, startTime: '09:00', endTime: '10:00' }]} />);
    const end = document.getElementById('end-1-0');
    const values = [...end.options].filter((o) => !o.disabled).map((o) => o.value);
    expect(values[0]).toBe('09:30');
    expect(values.at(-1)).toBe('24:00');
    await userEvent.selectOptions(document.getElementById('start-1-0'), '11:00');
    expect(val()[0]).toEqual({ weekday: 1, startTime: '11:00', endTime: '11:30' });
  });
  it('second slot does not overlap the first and copy to all fills the week', async () => {
    render(<Harness initial={[{ weekday: 1, startTime: '09:00', endTime: '10:00' }]} />);
    await userEvent.click(within(screen.getByTestId('day-1')).getByRole('button', { name: '+ Add slot' }));
    const [a, b] = val();
    expect(b.startTime >= a.endTime).toBe(true);
    const startOpts = [...document.getElementById('start-1-1').options].map((o) => o.value);
    expect(startOpts).not.toContain('09:30');
    await userEvent.click(within(screen.getByTestId('day-1')).getByRole('button', { name: 'Copy to all' }));
...
describe('NavDrawer and Header', () => {
  it('drawer lists the role links and closes with the cross', async () => {
    const onClose = vi.fn();
    renderWithProviders(<NavDrawer open role="admin" onClose={onClose} />, { role: 'admin' });
    expect(screen.getByRole('link', { name: /Add Doctor/ })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /Doctor List/ })).toBeInTheDocument();
  ...
  it('header shows name, email, logout and hamburger', async () => {
    ...
    renderWithProviders(<Header onMenuClick={onMenu} onLogout={onLogout} />, { role: 'patient' });
    expect(screen.getByTestId('header-name')).toHaveTextContent('Rina Sen');
    expect(screen.getByTestId('header-email')).toHaveTextContent('rina@example.com');
    expect(document.querySelector('.logo-name')).toHaveTextContent('West Bengal Forum for Mental Health');
...
describe('AnimatedRoutes', () => {
  function Nav() { const n = useNavigate(); return <button type="button" onClick={() => n('/b')}>go</button>; }
  it('keeps the previous view mounted while it slides out', async () => {
    ...
    act(() => { vi.advanceTimersByTime(EXIT_MS + 200); });
    await waitFor(() => expect(screen.queryByText('View A')).not.toBeInTheDocument());
    vi.useRealTimers();
  });
});

describe('Redirect inside AnimatedRoutes', () => {
  function Probe() { const n = useNavigate(); return <button type="button" onClick={() => n('/register')}>to register</button>; }
  it('a redirect in the view that slides out does not fire again', async () => {
    ...
    expect(screen.getByTestId('route-layer-enter')).toHaveTextContent('Register view');
  });
});
...
describe('Captcha', () => {
  it('loads an image and reports the id with the typed text', async () => {
    ...
    await waitFor(() => expect(document.querySelector('canvas').dataset.captchaId).toBe('cap-123'));
    const input = screen.getByLabelText('Captcha');
    await userEvent.type(input, 'ab cd');
    expect(onChange).toHaveBeenLastCalledWith({ captchaId: 'cap-123', captchaText: 'abcd' });
