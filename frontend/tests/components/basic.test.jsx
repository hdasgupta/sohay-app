...
describe('Dropdown', () => {
  const docs = [{ id: 1, name: 'Dr A' }, { id: 2, name: 'Dr B' }];
  it('renders a disabled placeholder and processed labels', () => {
    render(<Dropdown options={docs} labelProcessor={(d) => d.name} placeholder="Choose doctor" label="Doctor" />);
  ...
  it('passes the whole selected object to onOptionSelected', async () => {
    const onSel = vi.fn();
    render(<Dropdown options={docs} labelProcessor={(d) => d.name} onOptionSelected={onSel} label="Doctor" />);
...
describe('Password', () => {
  it('shows each rule on its own line with pass/fail state', () => {
    const { rerender, container } = render(<Password value="abc" onChange={() => {}} />);
    expect(container.querySelectorAll('.pw-rules li, [data-rule]').length).toBeGreaterThanOrEqual(6);
    expect(screen.getByText('At least one lower case letter').closest('li')).toHaveClass(/ok|pass/);
    expect(screen.getByText('At least one upper case letter').closest('li')).not.toHaveClass(/ok|pass/);
    rerender(<Password value="Abcdef1!" onChange={() => {}} />);
    expect(screen.getByText('At least 8 characters long').closest('li')).toHaveClass(/ok|pass/);
  });
  it('toggles visibility with the eye button', async () => {
    render(<Password value="Secret1!" onChange={() => {}} id="pw" label="Password" />);
  ...
  it('confirm mode only checks matching', () => {
    render(<Password value="Abcdef1!" matchWith="Abcdef1!" onChange={() => {}} />);
    expect(screen.getByText('Both passwords match').closest('li')).toHaveClass(/ok|pass/);
...
describe('MessageBox', () => {
  it('shows typed messages and closes them after 10 seconds', () => {
    vi.useFakeTimers();
    render(<MessageBox />);
    act(() => { notify.success('Saved'); notify.error('Failed'); });
    expect(screen.getByText('Saved').closest('.mb-toast')).toHaveAttribute('data-type', 'success');
    expect(screen.getByRole('alert')).toHaveTextContent('Failed');
    expect(screen.getAllByTestId('mb-progress')[0].style.animationDuration).toBe('10000ms');
...
describe('Loader', () => {
  it('shows the hourglass with a message while a call runs', () => {
    ...
    act(() => loaderBus.emit({ type: 'start', id: 1, message: 'Booking your appointment...' }));
    expect(screen.getByLabelText('Loading')).toHaveTextContent('Booking your appointment...');
...
describe('ErrorHandler', () => {
  it('catches render errors and reports them to the message box', () => {
    ...
    render(<><MessageBox /><ErrorHandler><Boom /></ErrorHandler></>);
